...
Status ObiewServiceImpl::ElectNewCoordinator() {
    TIME_LOG << "[" << obiew_address_ << "] "
    ...
    ElectCoordinatorRequest elect_req;
    elect_req.set_coordinator(my_paxos_address_);
    EmptyMessage elect_resp;
    return my_paxos_stub->ElectCoordinator(&context, elect_req, &elect_resp);
}
